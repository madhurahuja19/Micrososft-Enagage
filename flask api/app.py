...
def main():
    ...
    if flask.request.method == 'POST':
        ...
        m_name = flask.request.args['movie_name']
        m_name = m_name.title()
#        check = difflib.get_close_matches(m_name,all_titles,cutout=0.50,n=1)
        if m_name not in all_titles:
            ...
            for i in range(len(result_final)):
                ...
                id.append(str(result_final.iloc[i][2]))

            return jsonify(id)
            
            # response = make_response(render_template('positive.html',movie_names=names,movie_date=dates,search_name=m_name))
